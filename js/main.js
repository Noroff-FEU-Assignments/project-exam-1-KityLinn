...
const listRequestMobile = (mobileData) => {
  const mobileContainer = document.querySelector(".mobile-content");
  mobileData.forEach((mobileArray) => {
    ...
    <div class="mobile-box-img">
       <img src="${mobileArray._embedded["wp:featuredmedia"][0].source_url}" alt="${mobileArray.title.rendered}">
    </div>
    <h3>${mobileArray.title.rendered}</h3>
    </a>`
    mobileContainer.appendChild(mobilePost);
  })
};

makeRequest(carousel_url, listRequestMobile);
